...
namespace DeliveryTimeShopify
{
    public class Logger
    {
        public static readonly string LOG_PATH = System.IO.Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "DeliveryTimeShopify.log");
        ...
        {
            if (string.IsNullOrEmpty(Config.Instance.WebHookUrl))
                return;

            try
            {
                string url = Config.Instance.WebHookUrl;
                if (url.EndsWith("/"))
